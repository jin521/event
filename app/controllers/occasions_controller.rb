  ...
  def index
    @occasions = Occasion.all.order("created_at DESC")
      ...
      response = HTTParty.get('http://api.eventfinda.com.au/v2/events.json?rows=20', :basic_auth => auth)
        ...
        if occasion.nil?
          Occasion.create :title => event['name'], :description => event['description'], :date_start => event['datetime_start'], :date_end => event['datetime_end'], :latitude => event['point']['lat'], :longitude => event['point']['lng'], :eventfinda_id => event['id'], :address => event['address'], :location => event['location_summary'], :image => image, :price => event['is_free'], :website => event['url'], :phone => event['phone']
        end
